...
import qualified CLasH.Translator (makeVHDLAnnotations)
...
main' postLoadMode dflags0 args flagWarnings = do
  ...
  let dflags1 = dflags0{ ghcMode   = mode,
      ...
      dflags1a | DoInteractive <- postLoadMode = imp_qual_enabled
               ...
               | DoVHDL        <- postLoadMode = imp_qual_enabled
               | otherwise                 = dflags1
...
doVHDL []    = ghcError (UsageError "no input files")
doVHDL srcs  = do
  let (filePaths, _) = unzip srcs
  let libdir = GHC.Paths.libdir
  liftIO $ CLasH.Translator.makeVHDLAnnotations libdir filePaths
  return ()
...
doMkDependHSMode, doMakeMode, doVHDLMode, doInteractiveMode, doAbiHashMode :: Mode
...
isVHDLMode :: PostLoadMode -> Bool
isVHDLMode DoVHDL = True
isVHDLMode _      = False
